  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Your class timetable from the saved routine</p>
      </div>

      {entries.length === 0 ? (
        <Card><CardContent className="py-12 text-center text-muted-foreground">
          <Calendar className="w-10 h-10 mx-auto mb-2 opacity-30" />
          <p>No routine assigned yet. Contact admin.</p>
        </CardContent></Card>
      ) : (
        <Card>
          <CardHeader>
            ...
            <table className="w-full text-xs border-collapse">
              <thead>
                <tr>
                  ...
                  {DAYS.map(d => <th key={d} className="border p-2 bg-muted text-center min-w-[120px]">{d}</th>)}
                </tr>
              </thead>
              <tbody>
                {PERIODS.map(p => (
                          ...
                          {cells.length === 0 ? (
                            ...
                            return (
                              <div key={i} className={`rounded p-1.5 mb-1 ${isLab ? 'bg-emerald-100 dark:bg-emerald-900/30 border border-emerald-300 dark:border-emerald-700' : 'bg-blue-100 dark:bg-blue-900/30 border border-blue-300 dark:border-blue-700'}`}>
                                ...
                                <div className="text-muted-foreground">{c.courses?.name}</div>
                                <div className="flex gap-1 mt-0.5 flex-wrap">
                                  <Badge variant="outline" className="text-[10px] px-1 py-0">{c.rooms?.number}</Badge>
                                  <Badge variant="secondary" className="text-[10px] px-1 py-0">{c.batches?.batch_name}</Badge>
